...
class GUIpart(object):    
    ...
    def drawGrid(self, event=None):
        ...
        y1 = 0
        y2 = canvas_height*2
        del_x = 20 #set the length of each grid
        del_y = 20
        num_x = x2 / del_x
        num_y = y2 / del_y
        # draw center (0,0)
    ...
    def toggleRobot(self, event=None):
        if self.vWorld.real_robot: #if currently on, then disable it
        ...
        else:# if current disabled, turn it on
            if (len(gRobotList) > 0):
...
class VirtualHamsterWorld(object):
    ...
    def create_world(self):       
        ...
        self.vworld.vrobot.a = -1.571 #works the same
    ...
    def update_virtual_world(self):
        ...
        d_factor = 0.94 # travel distance conversion
        #################################
        ...
        while not self.gQuit:
            ...
            if (self.vworld.real_robot):    # convert prox sensor readings to number of pixels for the red beams
                ...
                if (prox_l > noise_prox):# if in real robot mode
                    i = 0
                ...
                if (prox_r > noise_prox):
                    ...
                    vrobot.dist_r = False
            else:  #if in simulation           # simulate prox sensors
                self.vworld.get_vrobot_prox("left")
                self.vworld.get_vrobot_prox("right")

            if (self.vworld.real_robot):# if in real robot mode
