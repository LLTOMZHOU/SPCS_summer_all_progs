...
class BehaviorThreads(object):
	Threshold_border = 40   # if floor sensor reading falls equal or below this value, border is detected
	Threshold_obstacle = 30   # if prox sensor reading is equal or higher than this, obstacle is detected
	...
	def setMachine(self):
		...
		self.sm.add_state("TurningLeft", "balance", self.moving_forward, "KeepGoingForward")
		self.sm.add_state("TurningLeft", "free", self.moving_forward, "GoingForward")
		self.sm.add_state("TurningLeft", "obsRight", self.turning_right, "TurningRight")
		#self.sm.add_state("TurningLeft", "free", self.moving_forward, "GoingForward")
		self.sm.add_state("TurningRight", "obsRight", self.turning_right, "TurningRight")
		self.sm.add_state("TurningRight", "balance", self.moving_forward, "KeepGoingForward")
		self.sm.add_state("TurningRight", "free", self.moving_forward, "GoingForward")
		self.sm.add_state("TurningRight", "obsLeft", self.turning_left, "TurningLeft")
		#self.sm.add_state("TurningRight", "free", self.moving_forward, "GoingForward")
		self.sm.add_state("GoingForward", "obsLeft", self.turning_left, "TurningLeft")
		self.sm.add_state("GoingForward", "obsRight", self.turning_right, "TurningRight")

		self.sm.add_state("GoingForward", "free", self.moving_forward, "GoingForward")
		self.sm.add_state("KeepGoingForward", "border", self.atBorder, "AtBorder")
		self.sm.add_state("KeepGoingForward", "free", self.moving_forward, "GoingForward")
		self.sm.add_state("TurningLeft", "border", self.atBorder, "AtBorder")
		self.sm.add_state("GoingForward", "border", self.atBorder, "AtBorder")
		self.sm.add_state("TurningRight", "border", self.atBorder, "AtBorder")

		self.sm.add_state("AtBorder", "obsLeft", self.clean_it, "AtBorder")
		self.sm.add_state("AtBorder", "obsRight", self.clean_it, "AtBorder")
		self.sm.add_state("AtBorder", "balance", self.clean_it, "AtBorder")
		#if the stop_move finds it hasn't done three cleaning processes, it manually puts 'free' event to the queue
		self.sm.add_state("AtBorder", "free", self.turnaround_and_move, "GoingForward")
			...
			for robot in self.robot_list:
				if self.go and robot:
					...
					if (prox_l > BehaviorThreads.Threshold_obstacle or prox_r > BehaviorThreads.Threshold_obstacle):
						...
						q1.put(alert_event)
						count += 1
						#update movement every 3 ticks, to avoid being too sensitive?
						if (count % 3 == 0):
							if prox_l - prox_r > 3:
								obs_event = Event("obsLeft", [prox_l, prox_r])
								print "obs_left"
								q2.put(obs_event)
							elif prox_l - prox_r < -3:
								...
								balanceCount += 1
								if balanceCount == 3:
									balance_event = Event("balance", [])
									q2.put(balance_event)
									time.sleep(0.06)
	...
	def clean_it(self):
		...
		print "starting to clean" + str(trashCount)
		if trashCount == 3:
			'''
			...
			print "Cleaned One Trash"
			free_event = Event("free", [])
			self.motion_q.put(free_event)
			free_event = Event("free", [])
			self.motion_q.put(free_event)
			free_event = Event("free", [])
			self.motion_q.put(free_event)

		print "ending the cleaning process"
	'''
	def KeepGoingForward(self):
		if self.robot_list and self.go:
			for robot in self.robot_list:
	...
	def atBorder(self):
		if self.robot_list and self.go:
			for robot in self.robot_list:
				robot.set_wheel(0, -85)
				robot.set_wheel(1, -85)
				time.sleep(0.24)
		self.stop_move()

	def turnaround_and_move(self):
		#print "turn around and move"
		if self.robot_list and self.go:
			for robot in self.robot_list:
...
class GUI(object):
	...
	def robot_alert_handler(self, q):
		...
		self.cc.after(20, self.robot_alert_handler, (q))
